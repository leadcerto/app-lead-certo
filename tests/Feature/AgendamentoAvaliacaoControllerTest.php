...
class AgendamentoAvaliacaoControllerTest extends TestCase
{
    ...
    public function test_campanha_nao_aceita_perfil_de_outro_tenant(): void
    {
        $tenant       = Tenant::factory()->create();
        $outroTenant  = Tenant::factory()->create();
        $dono         = $this->usuarioDono($tenant);
        $perfilProprio = $this->criarPerfil($tenant);
        $this->criarTemplate($tenant);
        $this->usuarioAvaliador($tenant);
        $perfilAlheio = $this->criarPerfil($outroTenant);

        $response = $this->actingAs($dono)->post('/admin/gmb/agendamentos/campanha', [
            'perfil_ids'    => [$perfilProprio->id, $perfilAlheio->id],
            'data_agendada' => now()->addDay()->toDateString(),
        ]);

        $response->assertSessionHasErrors('perfil_ids.1');
        $this->assertDatabaseMissing('agendamentos_avaliacao', ['perfil_id' => $perfilAlheio->id]);
    }
    ...
    public function test_alertar_avaliadores_envia_email_para_pendentes(): void
    {
        ...
        $response->assertRedirect();
        Mail::assertSent(\App\Mail\AlertaAvaliadorMail::class, fn ($mail) => $mail->hasTo($avaliador->email));
    }
}
