...
- **Limitação real (achada na 2ª revisão final de branch, 2026-07-10): botões só disparam em colunas com sequência automática ativa.** O único gatilho de envio hoje é `SequenciaService::iniciarParaTicket()` — se o dono configurar botões numa coluna sem nenhuma `Sequencia` ativa (ou com sequência mas zero mensagens ativas), a tela deixa salvar normalmente, mas os botões nunca são enviados, sem nenhum aviso. Não é bug, é consequência direta da decisão do usuário de disparar "junto com a última mensagem da sequência" (Task 11) — mas precisa virar um aviso na UI de configuração (Task futura: mostrar alerta quando a coluna tem `button_settings` mas não tem `Sequencia` ativa) ou documentação clara pro usuário, senão vai parecer bug reportado em produção.
- **Botões de resposta rápida + CTA (link/ligação) misturados na mesma mensagem**: `enviarBotoesDaColuna()` deixa combinar `move_column`/`trigger_ia`/`opt_out` com `open_url`/`call` no mesmo array de `choices`, mas o WhatsApp historicamente não permite misturar botão de resposta rápida com botão CTA (URL/ligação) na mesma mensagem — são tipos de mensagem diferentes na especificação da Meta. Não testado contra o Uazapi real ainda. Antes de liberar em produção, validar com um envio real se a Uazapi aceita a mistura ou se degrada silenciosamente; se não aceitar, a UI (Task 10) precisa impedir misturar as duas famílias nos 3 slots de uma coluna.
