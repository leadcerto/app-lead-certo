...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-gray-100">
        <div>
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Agentes Humanos (Equipe)</h1>
            </div>
        ...
        <div class="flex items-center gap-2">
            <span class="text-xs font-semibold px-3.5 py-2 bg-blue-50 text-blue-700 rounded-xl border border-blue-100">
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-3xl border border-gray-200/80 shadow-sm overflow-hidden">
            ...
            <div class="px-6 py-4 bg-gray-50/70 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                    <h2 class="text-base font-bold text-gray-900 flex items-center gap-2">
                        {{ $grupo['titulo'] }}
                        <span class="text-xs font-bold px-2 py-0.5 bg-gray-200/80 text-gray-700 rounded-full">
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100/90 p-4 shadow-2xs hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 flex items-start gap-4">
                        @if($u->avatar_url)
                            <img src="{{ $u->avatar_url }}" class="w-12 h-12 rounded-xl object-cover ring-2 ring-gray-100 flex-shrink-0" alt="">
                        @else
                            <div class="w-12 h-12 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-600 text-white flex items-center justify-center text-lg font-bold flex-shrink-0 shadow-sm shadow-blue-200">
                        ...
                        <div class="min-w-0 flex-1 space-y-1">
                            <div class="flex items-center justify-between gap-1">
                                <h3 class="font-bold text-gray-900 text-sm truncate">{{ $u->nome }}</h3>
                            ...
                            <div class="flex flex-wrap gap-1 pt-1.5">
                                @foreach($u->cargos as $c)
                                    <span class="text-[10px] font-medium bg-blue-50 text-blue-700 px-2 py-0.5 rounded-md border border-blue-100/60">
                                        {{ $c->icone ?: '💼' }} {{ $c->nome }}
                                    </span>
                                @endforeach
                            </div>
                ...
                @else
                <p class="text-xs text-gray-400 italic">Nenhum membro cadastrado neste nível hierárquico.</p>
                @endif
            </div>
        </div>
        @endforeach
