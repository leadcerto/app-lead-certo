...
class AgendamentoService
{
        ...
        try {
            foreach ($matriz as $perfilId => $dias) {
                ...
                foreach ($dias as $diaNome => $quantidade) {
                    ...
                    if ($diaOffset === null || $quantidade <= 0) continue;
    ...
    /**
     * Enviar Agora (Campanhas): dispara avaliações massivas
     * para uma data específica em vários perfis.
     *
     * @param array  $perfilIds IDs dos perfis selecionados
     * @param Carbon $data      Data para os agendamentos
     * @param int    $tenantId  Tenant do admin autenticado — cada perfil precisa
     *                          pertencer a ele
     * @return array ['criados' => int, 'avisos' => string[]]
     */
    public function enviarAgora(array $perfilIds, Carbon $data, int $tenantId): array
    {
        $criados = 0;
        $avisos  = [];
        ...
        try {
            foreach ($perfilIds as $perfilId) {
                ...
                try {
                    ...
                    $avisos[] = "⚠️ {$perfil->nome}: {$e->getMessage()}";
                }
            }
